...
def initialize_compiler(root_widget, get_current_tab_func, show_console_func, hide_console_func, pdf_monitor_setting="Default"):
    ...
    logs_console.log("LaTeX Compiler module initialized.", level='INFO')
...
def clean_project_directory(event=None):
    ...
    """
    logs_console.log("Clean Project Directory command initiated.", level='ACTION')
    ...
    if not current_tab or not current_tab.file_path:
        ...
        logs_console.log("Clean project failed: No active file path available.", level='WARNING')
        return
            ...
            if any(filename.endswith(ext) for ext in extensions_to_delete):
                ...
                try:
                    if os.path.exists(file_path_to_delete):
                        ...
                        logs_console.log(f"Successfully deleted auxiliary file: {file_path_to_delete}", level='DEBUG')
                except OSError as e:
                    logs_console.log(f"Error deleting file '{file_path_to_delete}': {e}", level='ERROR')

        if files_deleted_count > 0:
            logs_console.log(f"Successfully cleaned {files_deleted_count} auxiliary file(s).", level='SUCCESS')
            messagebox.showinfo(
                "Project Cleaned",
                f"Successfully deleted {files_deleted_count} auxiliary file(s) from:\n\n{project_directory}"
            )
        else:
            logs_console.log("No auxiliary files found to clean in the project directory.", level='INFO')
...
def compile_latex(event=None):
    ...
    Args:
        event (tk.Event, optional): The Tkinter event object, if called from a binding. Defaults to None.
    """
    logs_console.log("LaTeX compilation process started.", level='ACTION')
    current_tab = get_current_tab()
    if not current_tab:
        logs_console.log("LaTeX compilation aborted: No active editor tab.", level='WARNING')
        return
    ...
    if current_tab.file_path:
        ...
        try:
            ...
            logs_console.log(f"Created temporary file '{tex_file_path}' for compilation.", level='DEBUG')
        except Exception as e:
            messagebox.showerror("Error Saving Temp File", f"Could not save temporary file for compilation:\n{e}")
            logs_console.log(f"Error saving temporary file for compilation: {e}", level='ERROR')
            return
    ...
    try:
        ...
        try:
            ...
            logs_console.log("Log file not found, proceeding with empty log.", level='WARNING')
        ...
        if result.returncode == 0:
            ...
            try:
                ...
                if hasattr(state, 'debug_coordinator') and state.debug_coordinator:
                    ...
                    logs_console.log("Compilation result handled by debug system", level='INFO')
            except Exception as e:
            ...
            try:
                ...
                logs_console.log(f"Failed to cache successful .tex file: {e}", level='ERROR')
            ...
            try:
                ...
                try:
                    ...
                    logs_console.log(f"Error in fallback error display: {e2}", level='ERROR')
        ...
        logs_console.log("pdflatex command not found.", level='ERROR')
    except subprocess.TimeoutExpired:
        messagebox.showerror("Error", "LaTeX compilation timed out (exceeded 120 seconds).")
        logs_console.log("LaTeX compilation timed out.", level='ERROR')
    except Exception as e:
        ...
        if temp_file_created and os.path.exists(tex_file_path):
             try:
                 ...
                 logs_console.log(f"Removed temporary compilation file: {tex_file_path}", level='DEBUG')
             except OSError as e:
                 logs_console.log(f"Error removing temporary compilation file '{tex_file_path}': {e}", level='ERROR')
...
def display_pdf(pdf_path):
    ...
    if not pdf_path or not isinstance(pdf_path, str):
        ...
        logs_console.log("Invalid PDF path provided for display.", level='ERROR')
        return
        
    if not os.path.exists(pdf_path):
        messagebox.showerror("Error", f"PDF file not found at:\n{pdf_path}")
        logs_console.log(f"PDF file not found for display: {pdf_path}", level='ERROR')
        return
        ...
        if platform.system() == "Windows" and os.path.exists(sumatra_pdf_path):
            ...
            try:
                ...
                logs_console.log(f"Opened PDF with system default viewer: {pdf_path}", level='INFO')
                 ...
                 logs_console.log(f"Error opening PDF with system default viewer: {e}", level='ERROR')
    except Exception as e:
        messagebox.showerror("Error", f"Unexpected error while opening PDF: {e}")
        logs_console.log(f"Unexpected error while opening PDF: {e}", level='ERROR')
...
def view_pdf_external(event=None, pdf_path=None):
    ...
    """
    logs_console.log("View PDF externally command initiated.", level='ACTION')
    ...
    try:
        if pdf_path is None:
            ...
            if not current_tab or not current_tab.file_path:
                ...
                logs_console.log("View PDF failed: No active file path.", level='WARNING')
                return
        ...
        if platform.system() == "Windows" and os.path.exists(sumatra_pdf_path):
            ...
            if _pdf_monitor_setting != "Default":
                try:
                    ...
                    logs_console.log(f"Could not parse monitor setting '{_pdf_monitor_setting}'.", level='WARNING')
            ...
            try:
                ...
                logs_console.log(f"Opening PDF with SumatraPDF command: {' '.join(command)}", level='INFO')
            except Exception as e:
                messagebox.showerror("Error Opening PDF", f"Could not open PDF with SumatraPDF: {e}")
                logs_console.log(f"Error opening PDF with SumatraPDF: {e}", level='ERROR')
        else:
        ...
        logs_console.log(f"Unexpected error while viewing PDF: {e}", level='ERROR')

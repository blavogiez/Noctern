...
class InteractiveGridSelector(tk.Frame):
    ...
    def __init__(self, parent, on_selection_change: Callable[[int, int], None]):
        ...
        canvas_width = self.max_cols * (self.cell_size + self.cell_gap) + self.cell_gap
        canvas_height = self.max_rows * (self.cell_size + self.cell_gap) + self.cell_gap
    ...
    def _draw_grid(self):
        ...
        for row in range(self.max_rows):
            for col in range(self.max_cols):
                x1 = col * (self.cell_size + self.cell_gap) + self.cell_gap
                y1 = row * (self.cell_size + self.cell_gap) + self.cell_gap
                ...
                if row < self.selected_rows and col < self.selected_cols:
                    ...
                    color = "#f0f0f0"  # Light gray for unselected
                    outline = "#ddd"
    ...
    def _on_mouse_move(self, event):
        """Handle mouse movement over grid."""
        # Calculate which cell we're over
        col = min(self.max_cols, max(1, (event.x - self.cell_gap) // (self.cell_size + self.cell_gap) + 1))
        row = min(self.max_rows, max(1, (event.y - self.cell_gap) // (self.cell_size + self.cell_gap) + 1))
        ...
        if row != self.selected_rows or col != self.selected_cols:
            ...
            self._draw_grid()
            self.status_label.config(text=f"{self.selected_rows} × {self.selected_cols} table")
...
class TableInsertionPanel(BasePanel):
    ...
    def get_layout_style(self) -> PanelStyle:
        """Use scrollable layout for comprehensive table options."""
        return PanelStyle.SCROLLABLE
    
    def create_content(self):
        """Create the table insertion panel with interactive grid."""
        # Use scrollable layout from main_container
        scrollable_frame, canvas = self.main_container
        
        # Grid selector section
        self._create_grid_selector_section(scrollable_frame)
        
        # Table options section
        self._create_options_section(scrollable_frame)
        
        # Preview section
        self._create_preview_section(scrollable_frame)
    ...
    def _create_grid_selector_section(self, parent):
        """Create the interactive grid selector section."""
        grid_section = StandardComponents.create_section(parent, "Select Table Size")
        grid_section.pack(fill="x", pady=(0, StandardComponents.SECTION_SPACING))
        ...
        self.grid_selector.pack(anchor="w")

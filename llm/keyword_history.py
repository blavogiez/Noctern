...
def _get_keyword_filepath(file_path: str) -> str | None:
    ...
    """
    if not file_path:
        logs_console.log("Cannot generate keyword filepath: No document path provided.", level='DEBUG')
        return None
    ...
    logs_console.log(f"Generated keyword filepath: {keyword_file_path}", level='DEBUG')
    ...
    try:
        with open(keyword_filepath, 'r', encoding='utf-8') as f:
            ...
            if isinstance(keywords, list):
                logs_console.log(f"Loaded {len(keywords)} keywords for '{os.path.basename(file_path)}' from {os.path.basename(keyword_filepath)}", level='INFO')
                return keywords
            else:
                logs_console.log(f"Invalid keyword file format in '{keyword_filepath}': expected a JSON list.", level='WARNING')
                return []
    except (json.JSONDecodeError, IOError) as e:
        logs_console.log(f"Error reading keyword file '{keyword_filepath}': {e}", level='ERROR')
        return []
...
def set_keywords_for_file(file_path: str, keywords: list[str]):
    ...
    """
    if not file_path:
        logs_console.log("Cannot set keywords for an empty file path.", level='WARNING')
        return

    keyword_filepath = _get_keyword_filepath(file_path)
    if not keyword_filepath:
        logs_console.log("Could not determine keyword file path. Aborting save.", level='ERROR')
        return

    logs_console.log(f"Setting keywords for '{os.path.basename(file_path)}': {keywords}", level='CONFIG')
    try:
        with open(keyword_filepath, 'w', encoding='utf-8') as f:
            json.dump(keywords, f, indent=4)
        logs_console.log(f"Saved keywords to {os.path.basename(keyword_filepath)}", level='INFO')
    except IOError as e:
        logs_console.log(f"Error saving keyword file '{keyword_filepath}': {e}", level='ERROR')

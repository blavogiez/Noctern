"""
Intelligent LaTeX analysis engine using LLM.
Integrates with existing LLM system following SOLID principles.
"""
...
from debug_system.interfaces.analysis_engine import (
    IAnalysisEngine, AnalysisResult, DebugContext, QuickFix
)


class LLMAnalysisEngine(IAnalysisEngine):
    """
    LaTeX analysis engine using LLM for intelligent error analysis.
    Integrates with the existing LLM system while following SOLID principles.
    """
    
    def __init__(self):
        self._analysis_cache: Dict[str, AnalysisResult] = {}
        self._is_analyzing = False
        debug_console.log("LLM Analysis Engine initialized", level='DEBUG')
    
    def analyze_errors(self, context: DebugContext) -> AnalysisResult:
        """Analyze LaTeX errors with intelligent suggestions."""
        ...
        if context.diff_content:
            return self.analyze_diff(context.diff_content, context.log_content or "")
        else:
            return self._analyze_errors_directly(context)
    
    def analyze_diff(self, diff_content: str, log_content: str = "") -> AnalysisResult:
        """Analyze diff content using the existing LLM system."""
        ...
        try:
            ...
            analysis_text = self._call_existing_llm_system(diff_content, log_content)
            ...
            debug_console.log(f"LLM analysis completed with confidence {result.confidence}", level='INFO')
            return result
    ...
    def _analyze_errors_directly(self, context: DebugContext) -> AnalysisResult:
        ...
        # Create a synthetic diff from errors
        error_summary = "\n".join([
            f"Line {error.line_number}: {error.message}"
            for error in context.errors
        ])
        
        # Use the diff analysis with error summary
        return self.analyze_diff(error_summary, context.log_content or "")
    
    def _call_existing_llm_system(self, diff_content: str, log_content: str) -> Optional[str]:
        ...
        try:
            ...
            # Extract added lines like the original system
            added_lines = self._extract_added_lines(diff_content)
            if not added_lines.strip():
                debug_console.log("No added lines found in diff", level='WARNING')
                # Use full diff if no added lines
    ...
    def _parse_llm_response(self, analysis_text: str, diff_content: str) -> AnalysisResult:
        ...
        try:
            # First, try to extract JSON like the original system
            json_data = None
            ...
            if json_match:
                try:
                    ...
                    debug_console.log("Successfully parsed JSON from code block", level='DEBUG')
                except json.JSONDecodeError:
                    debug_console.log("Failed to parse JSON from code block", level='WARNING')
            ...
            if not json_data:
                ...
                if json_match:
                    try:
                        ...
                        debug_console.log("Successfully parsed JSON from text", level='DEBUG')
                    except json.JSONDecodeError:
                        debug_console.log("Failed to parse JSON from text", level='WARNING')
            ...
            if json_data:
                ...
                # Generate quick fixes from the analysis with error handling
                try:
                ...
                # Fallback: create structured result from plain text
                debug_console.log("No JSON found, using plain text analysis", level='INFO')
                
                # Try to extract some basic information from plain text
    ...
    def _generate_quick_fixes(self, json_data: Dict[str, Any], diff_content: str) -> List[QuickFix]:
        ...
        if corrected_code:
            # Generate a "Apply Correction" quick fix
            quick_fixes.append(QuickFix(
                ...
                confidence=0.8,
                auto_applicable=False  # Require user confirmation for LLM fixes
            ))
        
        # Generate specific quick fixes based on common patterns
            ...
            try:
                ...
                if package_match and package_match.groups():
                    ...
                    quick_fixes.append(QuickFix(
                        ...
                        fix_type="insert",
                        target_line=1,  # Add to beginning
    ...
    def _extract_explanation_from_text(self, text: str) -> str:
        ...
        # Try to find explanation-like content
        lines = text.split('\n')
        explanation_lines = []
    ...
    def _generate_text_based_fixes(self, text: str) -> List[QuickFix]:
        ...
        # Common patterns in text responses
...
class AnalysisEngineFactory:
    ...
    @staticmethod
    def create_llm_engine() -> IAnalysisEngine:
        """Create LLM-based analysis engine."""
        return LLMAnalysisEngine()
    
    @staticmethod
    def create_best_available_engine() -> IAnalysisEngine:
        """Create the best available analysis engine."""
        llm_engine = LLMAnalysisEngine()
        if llm_engine.is_available():
            return llm_engine
        else:
            # Fallback to a simple rule-based engine if needed
            debug_console.log("LLM not available, using fallback engine", level='WARNING')
            return LLMAnalysisEngine()  # Will return basic results when LLM unavailable
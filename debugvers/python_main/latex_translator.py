...
try:
    ...
    debug_console.log("argostranslate module not found. Translation functionality will be disabled.", level='WARNING')
...
def initialize_translator(root_ref, theme_getter, status_message_func, active_editor_getter, active_filepath_getter):
    ...
    debug_console.log("Translator service initialized.", level='INFO')
...
def _get_available_translation_pairs():
    ...
    if not _ARGOS_TRANSLATE_AVAILABLE:
        return []
    try:
        debug_console.log("Fetching available Argos Translate packages.", level='DEBUG')
        ...
        debug_console.log(f"Error getting Argos Translate packages: {e}", level='ERROR')
        return []
...
def _install_language_package_threaded(from_code, to_code, dialog_window, callback):
    ...
    def run_install():
        debug_console.log(f"Starting installation for package {from_code}->{to_code}.", level='INFO')
        try:
            ...
            debug_console.log(f"Successfully installed package {from_code}->{to_code}.", level='SUCCESS')
...
def _perform_translation_threaded(source_text, from_code, to_code, original_filepath, dialog_window):
    ...
    def run_translation():
        debug_console.log(f"Starting translation from {from_code} to {to_code}.", level='INFO')
        try:
            ...
            debug_console.log(f"Translation complete. Saving to {translated_filepath}", level='INFO')
            with open(translated_filepath, "w", encoding="utf-8") as f:
                f.write(translated_text)

            debug_console.log("Translation saved successfully.", level='SUCCESS')
...
def open_translate_dialog():
    ...
    """
    debug_console.log("Translate dialog opened.", level='ACTION')
    ...
    if not editor or not _root or not _theme_setting_getter_func or not _show_temporary_status_message_func:
        debug_console.log("Translator service not fully initialized. Aborting dialog.", level='ERROR')
    ...
    if not source_text.strip():
        debug_console.log("Translate dialog: Editor is empty.", level='INFO')
    ...
    if not display_options:
        debug_console.log("No Argos Translate packages found.", level='ERROR')
    ...
    def on_translate_button_click():
        ...
        debug_console.log(f"User selected translation pair: {from_code} -> {to_code}", level='ACTION')
        ...
        if not is_installed:
            debug_console.log(f"Package {from_code}->{to_code} is not installed. Prompting user.", level='INFO')
            ...
            if response:
                debug_console.log("User chose to install package.", level='ACTION')
                ...
                debug_console.log("User declined package installation.", level='ACTION')
                dialog.destroy()
            return

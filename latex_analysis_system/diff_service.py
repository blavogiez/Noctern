...
class CachedDiffGenerator(DiffGenerator):
    ...
    def __init__(self):
        ...
        logs_console.log("Cached diff generator initialized", level='DEBUG')
    ...
    def analyze_current_vs_last_successful(self, file_path: str, current_content: str) -> Tuple[bool, Optional[str], Optional[str]]:
        ...
        """
        logs_console.log(f"Analyzing diff for {file_path}", level='INFO')
        
        if not file_path:
            logs_console.log("No file path provided for diff analysis", level='WARNING')
        ...
        try:
            ...
            if not os.path.exists(cached_file_path):
                logs_console.log(f"No cached version found at {cached_file_path}", level='INFO')
                return False, None, None
            ...
            logs_console.log(f"Loaded cached version from {cached_file_path} ({len(last_content)} chars)", level='DEBUG')
            ...
            if not diff_lines:
                logs_console.log("No differences found between versions", level='INFO')
                return True, "", last_content
            
            diff_content = ''.join(diff_lines)
            logs_console.log(f"Generated diff ({len(diff_content)} chars)", level='DEBUG')
            
            logs_console.log("Differences found - diff generated", level='INFO')
            return True, diff_content, last_content
            
        except Exception as e:
            logs_console.log(f"Error analyzing diff: {e}", level='ERROR')
            return False, None, None
    
    def display_diff(self, diff_content: str, parent_window=None):
        """Display diff content to user."""
        if not diff_content.strip():
            logs_console.log("No diff content to display", level='DEBUG')
            return
        ...
        try:
            ...
            logs_console.log(f"Error displaying diff: {e}", level='ERROR')
...
class DiffViewer:
    ...
    def __init__(self, parent_window=None):
        ...
        logs_console.log("Diff viewer initialized", level='DEBUG')
    ...
    def show_diff(self, diff_content: str, title: str = "Version Comparison"):
        ...
        try:
            ...
            logs_console.log(f"Displayed diff with {len(diff_content)} characters", level='INFO')
            
        except Exception as e:
            logs_console.log(f"Error showing diff: {e}", level='ERROR')
    ...
    def _create_diff_window(self, title: str):
        ...
        logs_console.log("Diff window created", level='DEBUG')
    ...
    def _close_window(self):
        ...
        if self.diff_window:
            ...
            self.diff_window = None
        logs_console.log("Diff window closed", level='DEBUG')
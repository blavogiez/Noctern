...
class LLMAnalyzer(AnalysisEngine):
    ...
    def __init__(self):
        ...
        logs_console.log("LLM analyzer initialized", level='DEBUG')
    ...
    def is_available(self) -> bool:
        ...
        try:
            ...
            logs_console.log(f"LLM availability check failed: {e}", level='WARNING')
            return False
    ...
    def _analyze_diff(self, diff_content: str, log_content: str) -> AnalysisResult:
        ...
        if cache_key in self._analysis_cache:
            logs_console.log("Using cached analysis result", level='DEBUG')
            return self._analysis_cache[cache_key]
        
        logs_console.log("Starting LLM analysis of diff", level='INFO')
        ...
        try:
            ...
            logs_console.log(f"LLM analysis completed with confidence {result.confidence:.1%}", level='INFO')
            return result
            
        except Exception as e:
            logs_console.log(f"Error during LLM analysis: {e}", level='ERROR')
            return AnalysisResult(
    ...
    def _call_llm_system(self, diff_content: str, log_content: str) -> Optional[str]:
        ...
        try:
            ...
            if not prompt_template:
                logs_console.log("debug_latex_diff prompt template not found", level='ERROR')
                return None
            ...
            logs_console.log("Sending request to LLM", level='DEBUG')
            ...
            if response.get("success"):
                ...
                logs_console.log(f"LLM response received ({len(analysis_text)} chars)", level='DEBUG')
                return analysis_text
            else:
                error_msg = response.get("error", "Unknown LLM error")
                logs_console.log(f"LLM request failed: {error_msg}", level='ERROR')
                return None
                
        except Exception as e:
            logs_console.log(f"Error calling LLM system: {e}", level='ERROR')
            return None
    ...
    def _parse_llm_response(self, analysis_text: str, diff_content: str) -> AnalysisResult:
        ...
        logs_console.log(f"Parsing LLM response ({len(analysis_text)} chars)", level='DEBUG')
        ...
        try:
            ...
            if json_data:
                ...
                logs_console.log("No JSON found, using plain text analysis", level='INFO')
            ...
            logs_console.log(f"Error parsing LLM response: {e}", level='ERROR')
            return AnalysisResult(

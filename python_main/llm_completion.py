...
def request_llm_to_complete_text():
    ...
    if not (llm_state._completion_prompt_template or (llm_state._global_default_prompts and llm_state._global_default_prompts.get("completion"))): messagebox.showerror("LLM Service Error", "LLM prompt templates are not initialized. Please reload your file or restart the application."); return
    if llm_state._is_generating: interface.show_temporary_status_message("LLM is already generating. Please wait."); return
    ...
    # Get the safe callbacks, noting this is a completion for special post-processing
